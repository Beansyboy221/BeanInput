"""Raw mouse listener for Windows (Win32 Raw Input)."""
...
class RawMouseListener(base.BaseMouseListener):
    ...
    def _run(self):
        ...
        window_class.lpszClassName = f"RawMouseInput_{id(self)}"  # Unique class name
        ...
        raw_input_device.dwFlags = 0x00000100  # RIDEV_INPUTSINK
        raw_input_device.hwndTarget = self._hwnd

        if not self._user32.RegisterRawInputDevices(
            ctypes.byref(raw_input_device), 1, ctypes.sizeof(raw_input_device)
        ):
    ...
    def stop(self):
        ...
        try:
            win32gui.UnregisterClass(
                f"RawMouseInput_{id(self)}", win32gui.GetModuleHandle(None)
            )
        except Exception:
            pass
